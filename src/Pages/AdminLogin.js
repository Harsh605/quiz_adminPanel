...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const data = await axios.post(`${process.env.REACT_APP_API_URL}/admin/log-in`, payload);
    if (data.data.success) {
      setEmail('');
      setPassword('');
      localStorage.setItem('adminToken', data.data.data.user.jwt)
      localStorage.setItem('admin', JSON.stringify(data.data.data.user))
