...
const AdminForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const data = await axios.post(`${process.env.REACT_APP_API_URL}/admin/forgot-password`, payload);
    if (data.data.success) {
      setEmail('');

      navigate('/verify-otp', { state: { email: email, text: 'admin-forgot-password' } })
      setTimeout(() => {
