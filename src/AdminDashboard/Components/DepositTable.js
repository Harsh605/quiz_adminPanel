...
const DepositTable = () => {
    ...
    const getDepositList = async () => {
        ...
        if (adminToken) {
            ...
            const data = await axios.get(`${process.env.REACT_APP_API_URL}/admin/deposit-list`, { params: query, headers: { Authorization: adminToken } });
            if (data.data.success) {
                setUser(data.data.data.allDepositList)
    ...
    const updateFunction = (type, itemId) => {
        ...
        if (confirm) {
            ...
            axios.post(`${process.env.REACT_APP_API_URL}/admin/update-deposit-request`,
                {
                    type: type,
                    itemId: itemId
                },
                            ...
                            >
                                ...
                                <td style={{ display: 'flex', marginTop: '8px', height: '4rem', paddingLeft: '2rem' }}>
                                    ...
                                    {item.status !== 0 && <div className="col-12 col-lg-1 mt-1">
                                        <button className="btn btn-sm btn-primary" disabled style={{ width: '79px', fontSize: '15px', borderRadius: '5px', height: '30px', color: item.status === 1 ? 'green' : 'red' }}>{item.status === 1 ? 'Paid' : 'Rejected'}</button>
                                    </div>}
