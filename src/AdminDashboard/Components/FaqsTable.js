...
const FaqsTable = () => {
    ...
    const getFaqsList = async () => {
        ...
        if (adminToken) {
            ...
            const data = await axios.get(`${process.env.REACT_APP_API_URL}/admin/faqs-list`, { params: query, headers: { Authorization: adminToken } });
            if (data.data.success) {
                setData(data.data.data.allFaqs)
            ...
            <table className="w-full whitespace-nowrap">
                <thead>
                    <tr className="h-16 w-full text-sm leading-none text-gray-800">
                        ...
                        <th className="font-normal text-left pl-12">Faq Question</th>
                        <th className="font-normal text-left pl-12">Faq Answer</th>
                        ...
                        >
                            ...
                            <td className="pl-12">
                                <p className="text-sm font-medium leading-none text-gray-800">
                                    <Popover>
                                        <PopoverHandler>
                                            <p className='cursor-pointer text-sm font-medium leading-none text-gray-800'> {member?.question?.slice(0, 8)}...</p>
                                        </PopoverHandler>
